...
def resolve_log_path(tasks_path: str) -> str:
    directory = os.path.dirname(os.path.abspath(tasks_path))
    if not directory:
        directory = os.getcwd()
    return os.path.join(directory, "taskmaster_cli.log")
...
def tail_log(log_path: str, max_lines: int = 10) -> Iterable[str]:
...
def list_tasks(tasks_path: str, tag: Optional[str], only_available: bool, next_only: bool, log_path: str) -> int:
    ...
    if next_only:
        if not available:
            ...
            append_log(log_path, "next | none available")
            return 0
        cid, obj, _ = available[0]
        title = obj.get("title")
        status = obj.get("status")
        print(f"Next: {cid} | {status} | {title}")
        append_log(log_path, f"next | {cid} | {status} | {title}")
        return 0

    if only_available:
        if not available:
            print("No available tasks (all blocked or already in-progress/done).")
            append_log(log_path, "list-available | none")
            return 0
        for cid, obj, _ in available:
            title = obj.get("title")
            status = obj.get("status")
            print(f"{cid} | {status} | {title}")
        append_log(log_path, f"list-available | count={len(available)}")
        return 0
    ...
    append_log(log_path, "list-full")
    return 0


def set_status(tasks_path: str, tag: Optional[str], id_arg: str, status: str, log_path: str) -> int:
    status = status.strip()
    ...
    append_log(log_path, f"set-status | {id_arg} -> {status}")
    return 0
...
def main():
    ...
    log_path = resolve_log_path(args.file)
    recent = list(tail_log(log_path, max_lines=10))
    if recent:
        print('--- Recent Taskmaster activity ---')
        for line in recent:
            print(line.rstrip())
        print('----------------------------------')

    if args.cmd == "list":
        sys.exit(list_tasks(args.file, args.tag, args.available, args.next_only, log_path))
    if args.cmd == "next":
        sys.exit(list_tasks(args.file, args.tag, only_available=True, next_only=True, log_path=log_path))
    if args.cmd == "set-status":
        sys.exit(set_status(args.file, args.tag, args.id, args.status, log_path))
