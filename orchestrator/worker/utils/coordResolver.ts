...
export function resolveCoords(req: ResolveRequest): ResolveResult {
  ...
  if (req.mode.startsWith('block')) {
    if (!req.beginId || !req.endId) {
      return err('beginId and endId required for block modes');
    }
    ...
    let b = -1, e = -1;
    for (let i = 0; i < N; i++) {
      if (b < 0 && bRx.test(lines[i])) {
        b = i + 1;
      } // 1-based
      if (e < 0 && eRx.test(lines[i])) {
        e = i + 1;
      }
      if (b > 0 && e > 0) {
        break;
    ...
    if (b < 0) {
      return err(`BEGIN marker not found: ${req.beginId}`);
    }
    if (e < 0) {
      return err(`END marker not found: ${req.endId}`);
    }
    if (e < b) {
      return err('END occurs before BEGIN');
    }
  ...
  // Single anchor modes
  if (!req.anchor) {
    return err('anchor required for single-line modes');
  }
  ...
  if (line < 0) {
    return err(`Anchor not found: ${req.anchor}`);
  }
