...
function formatZodSchemaAsMarkdownFields(
  ...
  if (debug) {
    logger.debug(`[Format Data Path: object] Formatting object with keys: ${Object.keys(shape).join(', ')}`);
  }
  ...
  for (const key of Object.keys(shape)) {
    ...
    if (dataObject) {
      ...
      if (isOptional && (value === undefined || value === null || value === '') ) {
        if (debug) {
          logger.debug(`[Format Data Path: object.${key}] Skipping optional field with undefined value.`);
        }
...
function convertToPrimitive(value: any, schema: z.ZodTypeAny, debugInfo?: { path: string | string[], logger: any, debug: boolean }): any {
  ...
  if (typeof value !== 'string') {
    if (debug) {
      currentLogger.debug(`[Convert Path: ${pathStr}] Input is not a string (${typeof value}). Value: ${JSON.stringify(value)}`);
    }
    // Allow null/undefined through if schema permits
    if (value === null && schema instanceof z.ZodNullable) {
      return null;
    }
    if (value === undefined && schema instanceof z.ZodOptional) {
      return undefined;
    }
    if (value === null && schema instanceof z.ZodOptional) {
      return undefined;
    } // Treat null as undefined for optional
    ...
    if (baseSchema instanceof z.ZodString && typeof value === 'string') {
      return value;
    } // Already handled string case above
    ...
    if (debug) {
      currentLogger.debug(`[Convert Path: ${pathStr}] Non-string type (${typeof value}) doesn't match schema ${baseSchema.constructor.name}. Converting to string.`);
    }
  ...
  if (debug) {
    currentLogger.debug(`[Convert Path: ${pathStr}] Processing trimmed string value: "${stringValue.substring(0, 100)}..."`);
  }
  ...
  if (schema instanceof z.ZodOptional || schema instanceof z.ZodNullable || schema instanceof z.ZodDefault) {
    ...
    if (debug) {
      currentLogger.debug(`[Convert Path: ${pathStr}] Checking modifiers: Optional=${schema instanceof z.ZodOptional}, Nullable=${schema instanceof z.ZodNullable}, Default=${schema instanceof z.ZodDefault}. IsEmpty=${isEmpty}, IsNullish=${isNullish}`);
    }
    ...
    if (isNullish) {
      if (schema instanceof z.ZodDefault) {
        if (debug) {
          currentLogger.debug(`[Convert Path: ${pathStr}] Empty/nullish string for ZodDefault. Returning default value.`);
        }
  ...
  if (schema instanceof z.ZodString) {
    ...
    // This case should ideally be caught by ZodOptional/Nullable/Default handling above
    if (debug) {
      currentLogger.warn(`[Convert Path: ${pathStr}] Could not parse "${stringValue}" as boolean. Returning default (false).`);
    }
    ...
    if (debug) {
      currentLogger.warn(`[Convert Path: ${pathStr}] Value "${stringValue}" not in enum [${enumValues.join(', ')}]. Returning undefined.`);
    }
    return undefined; // No clear default for enum, let Zod validation handle it
  } else {
    // Fallback for unknown Zod types during conversion
    if (debug) {
      currentLogger.warn(`[Convert Path: ${pathStr}] Unhandled Zod type ${schema.constructor.name} in convertToPrimitive. Returning raw string value.`);
    }
    return stringValue;
  }
}
...
function extractPrimitiveValueFromNodes(nodes: MdNode[], debugInfo?: { path: string, logger: StructuredLogger, debug: boolean }): string {
  ...
  if (firstCodeBlock) {
    if (debug) {
      currentLogger.debug(`[Extract Path: ${path}] Strategy 1: Found code block. Returning its value.`);
    }
  ...
  if (nodes.length === 1 && listNodes.length === 1) {
    if (debug) {
      currentLogger.debug(`[Extract Path: ${path}] Strategy 2: Found a single list node.`);
    }
  ...
  if (debug) {
    currentLogger.debug(`[Extract Path: ${path}] Strategy 3: No code block or single list found. Stringifying relevant nodes.`);
  }
  ...
  if (stringified.startsWith('Description:')) {
    if (debug) {
      currentLogger.debug(`[Extract Path: ${path}] Stringified value seems to be only the description. Returning empty string.`);
    }
    return '';
  }

  if (debug) {
    currentLogger.debug(`[Extract Path: ${path}] Stringified value: "${stringified.substring(0, 100)}..."`);
  }
  return stringified;
}
...
function jaccardIndex<T>(setA: Set<T>, setB: Set<T>): number {
  if (setA.size === 0 && setB.size === 0) {
    return 1.0;
  } // Both empty, considered identical
  if (setA.size === 0 || setB.size === 0) {
    return 0.0;
...
function getSemanticSimilarity(
  ...
  if (!text1 && !text2) {
    return 1.0;
  } // Both empty, consider them identical
  if (!text1 || !text2) {
    return 0.0;
  ...
  if (tokens1.length === 0 && tokens2.length === 0) {
    return 1.0;
  } // Both become empty after preprocessing
  if (tokens1.length === 0 || tokens2.length === 0) {
    return 0.0;
  } // One becomes empty
...
function mapSectionToSchema(
  ...
  if (debug) {
    logger.debug(`[Map Path: ${currentPath}] > Section: "${sectionHeadingText}", Target Schema: ${schema.constructor.name}`);
  }
  ...
  if (baseSchema instanceof z.ZodObject) {
    ...
    if (debug) {
      logger.debug(`[Map Path: ${currentPath}] Iterating through ${section.children.length} children of section "${sectionHeadingText}" to find matches...`);
    }
    for (const childSection of section.children) {
      if (childSection.heading) {
        ...
        if (debug) {
          logger.debug(`[Map Path: ${currentPath}]  -- Checking child heading: "${childHeadingText}" (Normalized: "${normalizedChildHeading}")`);
        }
        ...
        for (const key of Object.keys(remainingKeys)) {
          ...
          if (debug) {
            logger.debug(`[Map Path: ${currentPath}]   - Comparing "${key}" (Normalized: "${normalize(key)}") with "${normalizedChildHeading}". Distance: ${keyDistance}`);
          }
        ...
        if (matchedKey) {
          ...
          if (debug) {
            logger.debug(`[Map Path: ${fieldPathStr}]   -- Normalized heading "${normalizedChildHeading}" matches schema key "${matchedKey}". Recursing.`);
          }
        ...
        if (debug) {
          logger.debug(`[Map Path: ${currentPath}]  -- Skipping child section with no heading.`);
        }
      }
    }
    if (debug) {
  ...
  if (baseSchema instanceof z.ZodArray) {
    ...
    if (directList && !(itemSchema instanceof z.ZodObject)) { // Only use for primitive arrays
      if (debug) {
        logger.debug(`[Map Path: ${currentPath}] Strategy 1: Found direct list in section nodes for primitive array (${directList.children.length} items).`);
      }
      directList.children.forEach((listItem, i) => {
        if (isListItem(listItem)) {
          ...
          if (debug) {
            logger.debug(`[Map Path: ${itemPath.join('.')}] Mapping list item ${i}.`);
          }
      ...
      if (results.length > 0) {
        if (debug) {
          logger.debug(`[Map Path: ${currentPath}] Returning ${results.length} items from direct list.`);
        }
        return results;
      }
    }
    ...
    if (debug) {
      logger.debug(`[Map Path: ${currentPath}] Strategy 2: Looking for item sections matching regex: ${itemHeadingRegex}`);
    }

    const itemSections: Section[] = [];
    const itemMatchPath = currentPath + `[${singularKeyName} items]`; // For logging
    if (debug) {
      logger.debug(`[ItemMatching Path: ${itemMatchPath}] Searching in parent section "${sectionHeadingText}" with ${section.children.length} children.`);
    }
    ...
    for (const childSection of section.children) {
      if (childSection.heading) {
        ...
        if (debug) {
          logger.debug(`[ItemMatching Path: ${itemMatchPath}]   - Checking child heading: "${headingText}". Regex match result: ${match}`);
        }
        if (match) {
        ...
        if (debug) {
          logger.debug(`[ItemMatching Path: ${itemMatchPath}]   - Child section has no heading. Skipping.`);
        }
      }
    }
    ...
    if (itemSections.length > 0) {
      if (debug) {
        logger.debug(`[Map Path: ${currentPath}] Strategy 2: Found ${itemSections.length} item sections based on headings matching '${singularKeyName} X'.`);
      }
      return itemSections.map((itemSec, i) => {
        const itemPath = [...path, String(i)];
        if (debug) {
          logger.debug(`[Map Path: ${itemPath.join('.')}] Mapping item section ${i} with heading "${mdastToString(itemSec.heading!)}".`);
        }
    ...
    if (slices.length > 0) {
      ...
      return slices.map((sliceNodes, i) => {
        ...
        if (debug) {
          logger.debug(`[Map Path: ${itemPath.join('.')}] Mapping thematic break slice ${i}.`);
        }
    ...
    if (!(itemSchema instanceof z.ZodObject)) {
      if (debug) {
        logger.debug(`[Map Path: ${currentPath}] Strategy 4: Handling array of primitives fallback.`);
      }
      ...
      if (looksLikeList) {
        ...
        if (debug) {
          logger.debug(`[Map Path: ${currentPath}] Parsing primitive array by splitting into ${lines.length} non-empty lines (non-list format).`);
        }
        return lines.map((ln, i) => {
          const itemPath = [...path, String(i)];
  ...
  if (debug) {
    logger.debug(`[Map Path: ${currentPath}] Extracted raw value: "${extractedValue.substring(0, 100)}${extractedValue.length > 100 ? '...' : ''}"`);
  }
  ...
  const convertedValue = convertToPrimitive(
    ...
    { path, logger: logger as ReturnType<typeof createLogger>, debug },
  );
  if (debug) {
    logger.debug(`[Map Path: ${currentPath}] Converted value: ${JSON.stringify(convertedValue)}`);
  }
  ...
  if (convertedValue === undefined && !(schema instanceof z.ZodOptional || schema instanceof z.ZodDefault)) {
    ...
    if (!(schema instanceof z.ZodNullable)) {
      if (debug) {
        logger.warn(`[Map Path: ${currentPath}] CONVERTED UNDEFINED for non-optional/non-default field.`);
      }
    }
  }
  if (convertedValue === null && !(schema instanceof z.ZodNullable || schema instanceof z.ZodDefault)) {
    ...
    if (!(schema instanceof z.ZodOptional)) {
      if (debug) {
        logger.warn(`[Map Path: ${currentPath}] CONVERTED NULL for non-nullable/non-default field.`);
      }
    }
  }
