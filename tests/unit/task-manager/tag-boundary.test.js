...
import {
	...
	useTag,
	deleteTag
} from '@tmstaging/scripts/modules/task-manager/tag-management.js';
...
describe('Tag boundary resolution', () => {
	...
	it('copyTag deep copy – mutation does not affect source', async () => {
		const { copyTag } = await import(
			'@tmstaging/scripts/modules/task-manager/tag-management.js'
		);
	...
	it('adds task to tag derived from state.json when no explicit tag supplied', async () => {
		...
		const { resolveTag } = await import('@tmstaging/scripts/modules/utils.js');
